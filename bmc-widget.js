(function() {
  ...
  if (window.__BMCWidget && window.__BMCWidget.BMC) return;
  window.__BMCWidget = window.__BMCWidget || {};
  window.__BMCWidget.BMC = true;
  ...
  const BMC_CONFIG = {
    ...
    phone: '918424895455',
    // Default language shifted to Marathi with focus on complaints
    welcome: '👋 नमस्कार! मी बृहन्मुंबई महानगरपालिकेचा (BMC) AI असिस्टंट आहे. तुम्ही येथे तुमच्या तक्रारी नोंदवू शकता. मला मराठी आणि इंग्रजी समजते. मी तुमची कशी मदत करू?',
  ...
  // Action Categories - Skipped Property Tax
  const actionItems = [
  ...
  async function initWidget() {
    const waBase = `https://wa.me/${BMC_CONFIG.phone}`;
    const defaultMsg = 'नमस्कार'; // Default Marathi greeting
    const qrImageUrl = generateQRUrl(BMC_CONFIG.phone, defaultMsg);

    // Create host element
    const host = document.createElement('div');
    document.body.appendChild(host);
    const shadow = host.attachShadow({ mode: 'open' });
    if (BMC_CONFIG.forceDark) host.classList.add('force-dark');
    ...
    tooltip.textContent = 'BMC AI Assistant'; 
    ...
    launcher.setAttribute('aria-label', 'Open BMC AI Assistant');
    launcher.innerHTML = `
    ...
    function openShelf() {
      ...
      shelf.setAttribute('aria-label', 'BMC AI Assistant');

      const greetKey = '__bmc_greeting_dismiss_' + BMC_CONFIG.orgId;
      const showGreeting = !BMC_CONFIG.suppressGreet && (BMC_CONFIG.forceGreet || !localStorage.getItem(greetKey));

      if (showGreeting) {
        const bubble = document.createElement('div');
        bubble.className = 'wa-bubble';
        bubble.innerHTML = '<button class="wa-dismiss" aria-label="Dismiss greeting">×</button>' + escapeHtml(BMC_CONFIG.welcome);
      ...
      const recentKey = '__bmc_recent_' + BMC_CONFIG.orgId;
      ...
      actionItems.forEach((item, idx) => {
        ...
        // Apply highlight class for designated items
        if (item.highlight) {
      ...
      qrPanel.innerHTML = `
        ...
        <div class="wa-scan-text">
          <strong>BMC AI शी चॅट करा!</strong><br>
          येथे तक्रार करा<br>
      ...
      footer.innerHTML = `
        <a class="wa-powered-link" href="${BMC_CONFIG.poweredByUrl}" target="_blank" rel="noopener" aria-label="Visit WoW-Strategies website">
          <span>Powered by <strong>${BMC_CONFIG.poweredBy}</strong></span>
    ...
    if (BMC_CONFIG.autoOpen) {
      const key = '__bmc_auto_opened_' + BMC_CONFIG.orgId;
      if (!sessionStorage.getItem(key)) {
        setTimeout(() => {
          openShelf();
          sessionStorage.setItem(key, '1');
        }, BMC_CONFIG.openDelay);
      }
    }
